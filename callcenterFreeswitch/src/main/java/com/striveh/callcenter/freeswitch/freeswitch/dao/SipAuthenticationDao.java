/** */
package com.striveh.callcenter.freeswitch.freeswitch.dao;

import org.springframework.stereotype.Repository;
import com.striveh.callcenter.common.base.dao.BaseDao;
import com.striveh.callcenter.pojo.freeswitch.SipAuthenticationPojo;

/**
 * @功能:【sip_authentication 】Dao
 * @项目名:callcenterFreeswitch
 * @作者:xxx
 * @日期:2020-04-06 10:35:31
 * @说明：<pre></pre>
 */
@Repository
public class SipAuthenticationDao extends BaseDao<SipAuthenticationPojo> {

}
